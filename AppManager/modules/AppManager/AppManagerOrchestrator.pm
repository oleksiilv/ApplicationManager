...
use AppManager::Models::AppServerManagersFactory
  qw( get_app_server_manager_model );
...
sub manage_app {
    ...
    my $app_server_manager_model = get_app_server_manager_model(
    ...
    switch ($action) {
        case "$ACTION_DEPLOY" {
            $status =
              get_deploy_app_service( app_server => $app_server )->deploy_app(
                app_server_manager_model => $app_server_manager_model,
        ...
        case "$ACTION_UNDEPLOY" {
            $status =
              get_undeploy_app_service( app_server => $app_server )
              ->undeploy_app(
                app_server_manager_model => $app_server_manager_model,
                application              => $application
              );
        ...
        case "$ACTION_START" {
            $status =
              get_start_app_service( app_server => $app_server )->start_app(
                app_server_manager_model => $app_server_manager_model,
                application              => $application
              );
        }
        case "$ACTION_STOP" {
            $status =
              get_stop_app_service( app_server => $app_server )->stop_app(
                app_server_manager_model => $app_server_manager_model,
                application              => $application
              );
        }
        case "$ACTION_CHECK" {
            $status =
              get_check_app_service( app_server => $app_server )->check_app(
                app_server_manager_model => $app_server_manager_model,
                application              => $application
              );
        }
    ...
    if ( $status->has_error_messages() ) {
        ...
        say "action '$action' succeeded with warning: "
          . $status->get_warning_messages_str;
    }
    else {
...
sub manage_configs {
    ...
    die
"both options config_filepath and save_to_config_filepath cannot be specified"
      if $config_filepath && $save_to_config_filepath;
    ...
    if ($config_filepath) {
        ...
        foreach my $option ( keys %options_hash ) {
            if ( $args->{$option} ) {
                die
"option '$option' is duplicated in config file and command line args. This behavior is not supported";
            }
            else {
